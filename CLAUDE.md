...
`sources/` holds the raw inputs, split by provenance: `sources/zotero/` (the Zotero export track), `sources/marc/` (the Z39.50 MARC-harvest track), and — at the `sources/` root — the two `*-crosswalk.csv` files that bridge them plus loose external reference data. `sources/Makefile` regenerates these (run it as `make -C sources <target>`, with paths subdir-qualified — e.g. `make -C sources zotero/artists-books.csv` or `make -C sources marc/reference-resources-marc.xml`).

`sources/zotero/zotero.sqlite` is the authoritative source. The `*_export.sh` scripts (in `sources/zotero/`) run `*_export.sql` against it to produce the CSVs the construct queries read; `notes_export.sh` emits `notes.xml`, and `sources/marc/marc_harvest.py` produces the `*-marc.xml` collections (with resumable state under `sources/marc/harvest/`, gitignored). The `tools/fuzzy-match/` matchers turn those into the `*-crosswalk.csv` files. The `-reconciled.csv` variants have been manually cleaned (e.g. mapped to Wikidata/ULAN IDs). See `sources/README.md` for the citation data model and the reference-works pipeline.
