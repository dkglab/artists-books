...
- `make graph/reference-resources.ttl` runs `queries/reference-resources.rq` over `sources/zotero/reference-resources.csv`, the note HTML archive (`sources/zotero/notes.zip`), and `abc-master-crosswalk.csv` to emit the `ab:ReferenceWork` nodes and `ab:Citation`s. The citation match is computed **live in SPARQL**: the query reads each "Cited:" note paragraph out of `notes.zip` (SPARQL-Anything archive + HTML triplifiers) and substring-matches the reference title against the flattened paragraph text (longest wins; an `<em>`-exact fallback for short titles) — this replaced the old `cite_match.py` + `citation-crosswalk.csv`. It also reads a **basic slice** of `sources/marc/reference-resources-marc.xml` — just the primary creator (`100 $a`), joined by `999 $a` like the books' MARC — emitted as a `bflc:PrimaryContribution` (#46). The rest of the reference MARC (OCLC/WorldCat, secondary creators, roles, identity URIs, extent/dimensions) is still pending (#40/#51).

Both are pure source-to-RDF transforms — they do **not** touch Fuseki. The committed inputs to the graph build (`abc-master-crosswalk.csv`, `notes.zip`, the `-marc.xml` files) are regenerated by `make -C sources …`; after regenerating them rebuild the graph explicitly with `make -B graph/reference-resources.ttl`, then re-run `make` to rebuild the site. Reading `notes.zip` adds ~4s to the reference build (the abc-crosswalk is joined *inside* the archive read so only in-scope note files are triplified — see `docs/QUERY-PERFORMANCE.md`).
...
`sources/zotero/zotero.sqlite` is the authoritative source. The `*_export.sh` scripts (in `sources/zotero/`) run `*_export.sql` against it to produce the CSVs the construct queries read; `notes_export.sh` emits `notes.zip` (one `<itemKey>.html` per "Cited:" note, read directly by `reference-resources.rq`), and `sources/marc/marc_harvest.py` produces the `*-marc.xml` collections (with resumable state under `sources/marc/harvest/`, gitignored). The remaining `tools/fuzzy-match/` matcher (`match.py`) turns those into `abc-master-crosswalk.csv`; the citation crosswalk it used to also produce is gone — that match is now done in SPARQL. The `-reconciled.csv` variants have been manually cleaned (e.g. mapped to Wikidata/ULAN IDs). See `sources/README.md` for the crosswalk pipeline (with `sources/zotero/README.md` for the Zotero database and citation data model, and `sources/marc/README.md` for the MARC harvest).
